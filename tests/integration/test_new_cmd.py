...
def test_new_creates_assignment_toml(public_root: Path, tmp_path: Path) -> None:
    ...
    cmd_new("my-assignment", template="python-stdin-stdout", staging_dir=staging, public_root=public_root)
...
def test_new_pipeline_scaffold_has_make_pipeline(public_root: Path, tmp_path: Path) -> None:
    staging = tmp_path / "staging"
    cmd_new("my-assignment", template="python-stdin-stdout", staging_dir=staging, public_root=public_root)
...
def test_new_rejects_invalid_slug(public_root: Path, tmp_path: Path) -> None:
    with pytest.raises(typer.Exit):
        cmd_new("MyAssignment", template="python-stdin-stdout", staging_dir=tmp_path / "s", public_root=public_root)
...
def test_new_force_overwrites(public_root: Path, tmp_path: Path) -> None:
    staging = tmp_path / "staging"
    cmd_new("my-assignment", template="python-stdin-stdout", staging_dir=staging, public_root=public_root)
    cmd_new("my-assignment", template="python-stdin-stdout", staging_dir=staging, force=True, public_root=public_root)
    assert (public_root / "assignments" / "my-assignment" / "assignment.toml").exists()
...
def test_new_assignment_toml_contains_slug(public_root: Path, tmp_path: Path) -> None:
    staging = tmp_path / "staging"
    cmd_new("my-assignment", template="python-stdin-stdout", staging_dir=staging, public_root=public_root)
    content = (public_root / "assignments" / "my-assignment" / "assignment.toml").read_text()
