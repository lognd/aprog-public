...
def _make_bundle(public_root: Path, private_root: Path, tmp_path: Path) -> Path:
    ...
    return cmd_package_private(
        ...
        output_dir=tmp_path / "dist", public_root=public_root,
    )


def test_intake_copies_solution(public_root: Path, private_root: Path, tmp_path: Path) -> None:
...
def test_intake_copies_grader(public_root: Path, private_root: Path, tmp_path: Path) -> None:
    bundle = _make_bundle(public_root, private_root, tmp_path)
    dest_private = tmp_path / "private"
    cmd_intake(bundle, public_repo=public_root, private_repo=dest_private)
...
def test_intake_copies_hidden_tests(public_root: Path, private_root: Path, tmp_path: Path) -> None:
    bundle = _make_bundle(public_root, private_root, tmp_path)
    dest_private = tmp_path / "private"
    cmd_intake(bundle, public_repo=public_root, private_repo=dest_private)
...
def test_intake_refuses_overwrite_without_force(public_root: Path, private_root: Path, tmp_path: Path) -> None:
    bundle = _make_bundle(public_root, private_root, tmp_path)
    dest_private = tmp_path / "private"
    cmd_intake(bundle, public_repo=public_root, private_repo=dest_private)
...
def test_intake_force_overwrites(public_root: Path, private_root: Path, tmp_path: Path) -> None:
    bundle = _make_bundle(public_root, private_root, tmp_path)
    dest_private = tmp_path / "private"
    cmd_intake(bundle, public_repo=public_root, private_repo=dest_private)
...
def test_intake_rejects_missing_bundle(public_root: Path, tmp_path: Path) -> None:
    with pytest.raises(typer.Exit):
        cmd_intake(tmp_path / "nonexistent.tar.gz", public_repo=public_root, private_repo=tmp_path / "priv")


def test_intake_rejects_unknown_assignment(public_root: Path, private_root: Path, tmp_path: Path) -> None:
    bundle = _make_bundle(public_root, private_root, tmp_path)
