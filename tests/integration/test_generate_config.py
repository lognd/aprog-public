...
def test_generate_config_creates_manifest(public_root: Path) -> None:
    ...
    manifest = public_root / "generated" / "assignments" / "linked-list-insertion" / "assignment-manifest.json"
    assert manifest.exists()


def test_generate_config_creates_run_autograder_py(public_root: Path) -> None:
    cmd_generate_config("linked-list-insertion", public_root=public_root)
    py = public_root / "generated" / "assignments" / "linked-list-insertion" / "run_autograder.py"
    assert py.exists()
...
def test_generate_config_creates_run_autograder_sh(public_root: Path) -> None:
    cmd_generate_config("linked-list-insertion", public_root=public_root)
    sh = public_root / "generated" / "assignments" / "linked-list-insertion" / "run_autograder"
    assert sh.exists()
...
def test_manifest_contains_source_hash(public_root: Path) -> None:
    cmd_generate_config("linked-list-insertion", public_root=public_root)
    manifest_path = public_root / "generated" / "assignments" / "linked-list-insertion" / "assignment-manifest.json"
