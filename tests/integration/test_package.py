...
def test_package_public_creates_tarball(public_root: Path, tmp_path: Path) -> None:
    out = cmd_package_public(
        "linked-list-insertion", output_dir=tmp_path, public_root=public_root
    )
    assert out.exists()
...
def test_package_public_contains_assignment_toml(
...
def test_package_private_creates_tarball(
...
def test_package_private_contains_pipeline(
    public_root: Path, private_root: Path, tmp_path: Path
) -> None:
    sol = private_root / "solutions" / "linked-list-insertion"
    ht = private_root / "hidden-tests" / "linked-list-insertion"
...
def test_package_private_contains_manifest(
    public_root: Path, private_root: Path, tmp_path: Path
) -> None:
    sol = private_root / "solutions" / "linked-list-insertion"
    ht = private_root / "hidden-tests" / "linked-list-insertion"
...
def test_package_private_fails_without_pipeline(
    public_root: Path, tmp_path: Path
) -> None:
